...
export function formatNotionBlock(block) {
  ...
  for (let i = 0; i < blocksToProcess.length; i++) {
    ...
    if (b?.value?.type === 'sync_block' && b?.value?.children) {
      ...
      childBlocks.forEach((childBlock, index) => {
        const blockValue =
          typeof childBlock === 'string' ? clonedBlock[childBlock] : childBlock
        if (!blockValue || typeof blockValue !== 'object') {
          return
        }
        const newBlockId = blockValue.value?.id || `${blockId}_child_${index}`
        clonedBlock[newBlockId] = blockValue
  ...
  Object.keys(clonedBlock || {}).forEach(blockId => {
    const blockValue = clonedBlock[blockId]
    if (!blockValue || typeof blockValue !== 'object' || !blockValue.value?.id) {
      delete clonedBlock[blockId]
    }
  })
